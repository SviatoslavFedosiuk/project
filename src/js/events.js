...
formRef.addEventListener("submit", async (e) => {
  ...
  const countryInput = e.currentTarget.elements.country.value
    .trim()
    .toLowerCase();
  ...
  if (countryInput) {
    const country = countries.find(({ name, code }) => {
      return (
        name.toLowerCase() === countryInput ||
        code.toLowerCase() === countryInput
      );
    });
